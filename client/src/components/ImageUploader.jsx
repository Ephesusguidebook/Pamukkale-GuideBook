  ...
  async function handleFiles(files) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const res = await api.post('/admin/tours/upload', formData, {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-3">
        {images.map((img, idx) => (
          <div key={img.url + idx} className="relative h-24 w-32 overflow-hidden rounded-lg border border-gray-200">
      ...
      <p className="mt-1 text-xs text-gray-400">İlk görsel kapak fotoğrafı olarak kullanılır.</p>
    </div>
  );
}
